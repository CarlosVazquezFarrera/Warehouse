...
export abstract class HttpCrudService<TEntity, TDto> {
  ...
  async getById(id: string): Promise<TEntity> {
    return lastValueFrom(this.http.get<TEntity>(`${this.apiUrl}/${id}`));
  }

  async create(dto: TEntity): Promise<TDto> {
    return lastValueFrom(this.http.post<TDto>(this.apiUrl, dto));
  }
  async createList(dtos: TEntity[]): Promise<TDto[]> {
    return lastValueFrom(this.http.post<TDto[]>(`${this.apiUrl}/List`, dtos));
  }
  async update(dto: TDto): Promise<TDto> {
    return lastValueFrom(this.http.put<TDto>(`${this.apiUrl}`, dto));
  }
